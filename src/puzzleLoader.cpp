...
vector<Puzzle> loadPuzzlesFromFile()
{
  ...
  while (getline(file, line)){
    ...
    if (theme.find("mate") != string::npos){
      mateIn = stoi(theme.substr(6, theme.length() - 6));
      allPuzzles.emplace_back(puzzleId, fen, firstMove, rating, theme, mateIn); // Use emplace_back instead of push_back for performance
